...
- **In-app server window**: clicking the footer's cloud badge opens a terminal-style window streaming the server's request log — the same metadata-only lines `docker logs` shows, held in a 300-line in-memory ring on the server and served at `/api/log`, polled every 2 seconds while the window is open (the poll excludes itself from the log it reads). Self-hosters can now answer "did my write land, when, and what did the server say" without shelling into the host. The manual flush-and-sync moved from the badge's click into the window's corner: as a flush it is nearly obsolete — typing debounces, structural operations save instantly, lock and unload drain, `Ctrl+S` remains — but it stays as the only retry lever after a failed write, since the design has no automatic retry. Mounting the Docker socket to read the real log was rejected for the same reason it always has been.
