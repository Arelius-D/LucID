...
- **In-app server panel**: clicking the footer's cloud badge opens a floating panel anchored on it, a context-menu family member like every other panel (no backdrop, dismissed by the next click anywhere else), showing the server's live request log: the same metadata-only lines `docker logs` shows, held in a 300-line in-memory ring on the server and served at `/api/log`, polled every 2 seconds while open (the poll excludes itself from the log it reads). Self-hosters can now answer "did my write land, when, and what did the server say" without shelling into the host. The manual sync is a Force sync control in the panel's top corner, wearing `lin-cloud-change`, and leaves the panel open so the write it forces appears in the log being watched: as a flush it is nearly obsolete (typing debounces, structural operations save instantly, lock and unload drain, `Ctrl+S` remains), but it stays as the only retry lever after a failed write, since the design has no automatic retry. Mounting the Docker socket to read the real log was rejected for the same reason it always has been.

### Fixed

- **Ciphertext could leak into the tag library and multiply**: a vault write that fired while the in-memory vault still held ciphertext, in the window between deriving the key and finishing decryption on unlock, encrypted already-encrypted strings a second time. Notes recover on the next save (they are rebuilt from plaintext every time), but the tag library is a union that never forgets, so `ENC:…` "tags" accumulated, each unlock adding a layer. The vault now carries an explicit plaintext invariant: `saveStore` refuses, loudly, unless decryption has completed since the last time ciphertext entered memory, and on every entry any remaining `ENC:`-prefixed tag remnants are purged from the library and from notes and the repair saved.
- **Emptying the trash now closes the trash panel**: an emptied panel had nothing left to show or act on, yet stayed open until the user clicked elsewhere.
- **No horizontal scrollbar in the trash panel**: a long folder or note name widened the panel's rows and, with only vertical overflow declared, forced a horizontal bar; the panel now clips sideways exactly as the sidebar tree does.
- **Icon and label never disagree on a state**: four rules straddled the stylesheet's two state models (content rows brighten to `--text-primary` on hover/active; controls take `--accent`) and painted an icon one colour and its label another. The trash row's glyph went accent while its text went primary; the view tabs used the row model on hover and the control model when active; the folder caret stayed muted while its row brightened; the import button's glyph went accent while importing and its label did not. Each now follows its own family: trash glyph and text together, view-tab hover in accent like its active state and the pills beside it, caret inheriting its row's colour once the row is hovered or active, importing label matching its glyph. Verified from computed colours with the theme's colour transitions settled; identical in all eight themes, since every theme block sets tokens only.
- **The import button no longer breaches the footer card at the narrowest sidebar**: the comfort cluster plus the labelled import button need 14.25rem of card width, and at the sidebar's 15rem floor the card offers 13.5rem, so the button pushed 0.75rem into the card's border. The footer is now a size container and the import button sheds its text label below the fitting width, becoming icon-only exactly like the Lock button that never had the problem; the label returns as soon as there is room.
- **Comfort-row glyphs no longer drift when the sidebar is resized**: the footer's top row laid its buttons directly on `space-between`, so dragging the divider redistributed the new width between every glyph, while the actions row below (two clusters with fixed gaps inside) held still. The comfort buttons now sit in a left cluster mirroring the actions row's shape: resizing stretches only the void between the cluster and the import corner.
...
- **Menus stop hiding what they mark**: the theme, font and auto-lock menus replaced the current entry's icon with a tick, so the active theme lost its own glyph: Amber Hour showed a tick instead of its sun-through-fog. Themes now keep their glyphs (sun, sun-through-fog, moon) and are listed light to dark; the font list marks the chosen set and leaves the rest clean; the auto-lock list marks the chosen duration with a watch-status glyph where it previously marked nothing. Menu glyphs were also sized to match the blank marker, so a menu with icons no longer sits taller than one without.
...
- **A refused passphrase now looks refused**: it cleared the field and shook it and that was all, so the only signal was motion, blink and you missed it, and with reduced motion switched on there was no shake to miss. The edge turns danger for exactly as long as the shake plays: colour says what happened, motion says that it happened, and neither survives into the next attempt. It is the same single line focus and validation use, never an extra ring, and there is no fill because the box is empty by then. The hold is read from the same token that drives the shake, so the two cannot drift, and it runs on a timer rather than `animationend`, which never fires with reduced motion, and never fires in a throttled background tab either.
- **A passphrase field draws ONE line, whatever is happening to it**: clicking one drew three concentric rings for a single fact, the edge recoloured to accent, a ring hugged that edge, and the app-wide keyboard-focus outline sat outside both. It looked like a double border because it *was* a double border, on both the first-run and the locked screens, and it hit mouse users too: browsers match `:focus-visible` on text inputs regardless of how you got there. Focus now recolours the edge and does nothing else; state keeps that same line and wins while it lasts, so clicking into a mismatched field no longer trades the error colour away for a focus colour. The detached outline stays where it earns its place, buttons and tree rows, which have no caret and no edge of their own.
...
- **Auto-lock's `Off` was a half-truth**: a 60-minute ceiling fires whatever the soft timer says, so the entry now reads `Off (60 minutes)` in both the menu and the tooltip. A duration under a minute is also representable at last: the stored value was read with `parseInt`, which floored anything smaller to zero and silently meant Off.
- **An active view tab no longer crowds its container**: the centre toolbar padded its tab strip to a third of what the explorer's pill strip uses, so an active tab sat three pixels inside its border where an active pill sits five inside its own. Same component family, two different insets. Both are one step of padding plus a hairline now.
...
- **The inspector's + button opened a menu that closed itself** in the same click (a missing propagation stop), and the context-menu engine had no height cap, so a long tag list would have run off the viewport.
