...
- **Ciphertext could leak into the tag library and multiply**: a vault write that fired while the in-memory vault still held ciphertext — the window between deriving the key and finishing decryption on unlock — encrypted already-encrypted strings a second time. Notes recover on the next save (they are rebuilt from plaintext every time), but the tag library is a union that never forgets, so `ENC:…` "tags" accumulated, each unlock adding a layer. The vault now carries an explicit plaintext invariant: `saveStore` refuses, loudly, unless decryption has completed since the last time ciphertext entered memory, and on every entry any remaining `ENC:`-prefixed tag remnants are purged from the library and from notes and the repair saved.
